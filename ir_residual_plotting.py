...
python ir_residual_plotting.py --path='./tweakreg_test11' --filter='160W' --new_or_old_version='old'
...
def residual_plotting_main(path,filter,new_or_old_version):
	...
	for file,im in zip(list_of_files, image): 	
		...
		#------------------------------------------------
			...
			ax0.set_xlim([np.min(table[:,0])-100, np.max(table[:,0])+100])
			ax0.set_ylim(ylim)
		...
		#------------------------------------------------
			...
			ax1.set_xlim([np.min(table[:,0])-100, np.max(table[:,0])+100])
			ax1.set_ylim(ylim)
		...
		#------------------------------------------------										
			...
			ax2.set_xlim([np.min(table[:,1])-100, np.max(table[:,1])+100])
			ax2.set_ylim(ylim)
		...
		#------------------------------------------------
			...
			ax3.set_xlim([np.min(table[:,1])-100, np.max(table[:,1])+100])
			ax3.set_ylim(ylim)
